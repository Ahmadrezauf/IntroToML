# -*- coding: utf-8 -*-
"""ns_IML_Task_3_7.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1zoVWdVknT2iSdpg44FTEjOazY22pZulp
...
ada = ADASYN(sampling_strategy = 0.06, random_state=42)

train_seqs_onehot, train_labels = ada.fit_resample(train_seqs_onehot, train_labels)
...
def create_model():
  model = Sequential()
  model.add(Dense(HIDDENSIZE, input_dim = 80, activation='relu', kernel_initializer= 'he_normal'))
  model.add(Dense(HIDDENSIZE, input_dim = HIDDENSIZE, activation='relu', kernel_initializer= 'he_normal'))
  model.add(Dense(1, input_dim = HIDDENSIZE, activation='sigmoid'))
...
kfold_splits = 5
folds = list(StratifiedKFold(n_splits=kfold_splits, shuffle=True, random_state=1).split(train_seqs_onehot, train_labels))
...
# model.summary()
callbacks = [ModelCheckpoint(filepath=mode_path, save_best_only=True)]
best_model.fit(train_seqs_onehot,train_labels, validation_split=0, epochs = NEPOCHS, batch_size=BATCHSIZE, verbose = 0 ,
            callbacks=callbacks, class_weight = class_weight_dict)  # starts training
